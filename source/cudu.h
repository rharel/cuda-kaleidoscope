...
public:
  ...
  explicit DataBlock(const size_t size = 0) : m_size(size), m_data(nullptr) {
    if (m_size > 0) {
  ...
  void
  download(T * const host, const size_t offset, const size_t extent) const {
...
public:
  ...
  static Array<T, rank> from_ptr(const T * data, const Shape<rank> shape) {
    Array<T, rank> array(shape);
    array.block().upload_all(data);
    return array;
  }

  explicit Array(const Shape<rank> & shape) : m_shape(shape), m_data(size()) {}
  ...
  void upload_single(
      const size_t i, const size_t j, const size_t k, const T value) const {
    CUDU_DEBUG_ASSERT(i < shape()[0]);
    CUDU_DEBUG_ASSERT(j < shape()[1]);
    CUDU_DEBUG_ASSERT(k < shape()[2]);
  ...
  void clear() { *this = cudu::device::Array<T, rank>(); }
...
public:
  ...
  __device__ const T & operator[](const size_t i) const { return block()[i]; }
...
public:
  ...
  __host__ __device__ const Shape<rank> & shape() const { return m_shape; }
