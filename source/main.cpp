...
void from_json(const nlohmann::json & json, kaleido::Keyframe & keyframe) {
  ...
  if (json.contains("mirror_internal_angle_degrees")) {
    keyframe.mirror_internal_angle_degrees =
        json["mirror_internal_angle_degrees"].get<float>();
  }
  if (json.contains("mirror_bend_angle_degrees")) {
    keyframe.mirror_bend_angle_degrees =
        json["mirror_bend_angle_degrees"].get<float>();
  }
  if (json.contains("background_texture_scale")) {
    keyframe.background_texture_scale =
        json["background_texture_scale"].get<float>();
  }
  if (json.contains("background_texture_offset")) {
    keyframe.background_texture_offset =
        json["background_texture_offset"].get<std::array<float, 2>>();
  }
  if (json.contains("background_texture_rotation_degrees")) {
    keyframe.background_texture_rotation_degrees =
        json["background_texture_rotation_degrees"].get<float>();
  }
  if (json.contains("camera_position_polar_angle_degrees")) {
    keyframe.camera_position_polar_angle_degrees =
        json["camera_position_polar_angle_degrees"].get<float>();
  }
  if (json.contains("medium_scattering_coefficient")) {
    keyframe.medium_scattering_coefficient =
        json["medium_scattering_coefficient"].get<float>();
  }
}
} // namespace kaleido
...
kaleido::Config config_from_json_file(const std::string & path) {
  ...
  config.mirror_internal_angle_degrees =
      json_config["mirror_internal_angle_degrees"].get<float>();
  config.mirror_bend_angle_degrees =
      json_config["mirror_bend_angle_degrees"].get<float>();
  ...
  config.background_texture_path =
      json_config["background_texture_path"].get<std::string>();
  config.background_texture_scale =
      json_config["background_texture_scale"].get<float>();
  config.background_texture_offset =
      json_config["background_texture_offset"].get<std::array<float, 2>>();
  config.background_texture_rotation_degrees =
      json_config["background_texture_rotation_degrees"].get<float>();
  config.medium_scattering_coefficient =
      json_config["medium_scattering_coefficient"].get<float>();
  config.medium_luminance_threshold =
      json_config["medium_luminance_threshold"].get<float>();
  config.camera_field_of_view_degrees =
      json_config["camera_field_of_view_degrees"].get<float>();
  config.camera_position_radius =
      json_config["camera_position_radius"].get<float>();
  config.camera_position_polar_angle_degrees =
      json_config["camera_position_polar_angle_degrees"].get<float>();
  config.camera_position_azimuth_angle_degrees =
      json_config["camera_position_azimuth_angle_degrees"].get<float>();
  config.stratifier_resolution =
      json_config["stratifier_resolution"].get<uint32_t>();
  config.stratifier_rng_seed =
      json_config["stratifier_rng_seed"].get<uint32_t>();
  config.nr_render_iterations =
      json_config["nr_render_iterations"].get<uint32_t>();
  ...
  config.animation_keyframes =
      json_config["animation_keyframes"].get<std::vector<kaleido::Keyframe>>();

  return config;
}

cudu::host::Array3D<unsigned char>
generated_image(const kaleido::Config & config) {
  cudu::host::Array3D<unsigned char> image = kaleido::raytraced_image(config);
  const cv::Mat image_cv(
      int(image.shape()[0]), int(image.shape()[1]), CV_8UC3, image.begin());
...
void run_gui_main_loop(
  ...
  while (cv::getWindowProperty(MAIN_WINDOW_TITLE, 0) >= 0 &&
    ...
    case KEYCODE_ITERATION_DECREASE:
      config.nr_render_iterations =
          std::max(config.nr_render_iterations / 2, 2u);
      render_request = true;
      break;
    ...
    case KEYCODE_STRATA_DECREASE:
      config.stratifier_resolution =
          std::max(config.stratifier_resolution / 2, 1u);
      render_request = true;
      break;
    default:
      break;
...
int main(int argc, char * argv[]) {
  ...
  if (config.animation_enabled) {
    ...
    for (const kaleido::Keyframe & keyframe : config.animation_keyframes) {
      nr_frames_total +=
          size_t(roundf(keyframe.duration_seconds * config.animation_fps));
    }
    for (const kaleido::Keyframe & keyframe : config.animation_keyframes) {
      const size_t nr_frames =
          size_t(roundf(config.animation_fps * keyframe.duration_seconds));

      kaleido::Keyframe step;

#define KALEIDO_KEYFRAME_STEP(field)                                           \
  step.field = keyframe.field == kaleido::Keyframe::CONSTANT                   \
                   ? 0                                                         \
...
#undef KALEIDO_KEYFRAME_STEP
      ...
      for (size_t i = 0; i < nr_frames; ++i) {
        ...
        const uint32_t approx_time_remaining_s =
            round(frame_time_average_s * nr_frames_remaining);
        std::cout << "Rendering frame " << i_frame << "/" << nr_frames_total
                  << " | "
                  << "Time remaining (approx): " << approx_time_remaining_s
                  << " seconds" << std::endl;

        config.mirror_internal_angle_degrees +=
            step.mirror_internal_angle_degrees;
        ...
        config.background_texture_offset[0] +=
            step.background_texture_offset[0];
        config.background_texture_offset[1] +=
            step.background_texture_offset[1];
        config.background_texture_rotation_degrees +=
            step.background_texture_rotation_degrees;
        config.camera_position_polar_angle_degrees +=
            step.camera_position_polar_angle_degrees;
        config.medium_scattering_coefficient +=
            step.medium_scattering_coefficient;
  ...
  std::thread render_thread(
      [&render_thread_stop, &render_request, &config, &image]() {
