...
equals_approx(const float a, const float b, const float epsilon = 1e-6) {
  return fabsf(a - b) < epsilon;
}

float radians(const float degrees) { return degrees * float(M_PI) / 180; }

#define VEC2_BINARY_OPERATOR(name, op)                                         \
  template <typename T>                                                        \
  __host__ __device__ Vec2<T> name(const Vec2<T> & v, const T scalar) {        \
    return Vec2<T>(v.x op scalar, v.y op scalar);                              \
  }                                                                            \
                                                                               \
  template <typename T>                                                        \
  __host__ __device__ Vec2<T> name(const T scalar, const Vec2<T> & v) {        \
    return Vec2<T>(scalar op v.x, scalar op v.y);                              \
  }                                                                            \
                                                                               \
  template <typename T>                                                        \
  __host__ __device__ Vec2<T> name(const Vec2<T> & v0, const Vec2<T> & v1) {   \
    return Vec2<T>(v0.x op v1.x, v0.y op v1.y);                                \
  }

#define VEC2_BINARY_ASSIGNMENT_OPERATOR(name, op)                              \
  __host__ __device__ Vec2 & name(const T scalar) {                            \
    return *this = *this op scalar;                                            \
  }                                                                            \
                                                                               \
  __host__ __device__ Vec2 & name(const Vec2 & other) {                        \
    return *this = *this op other;                                             \
  }

#define VEC2_BINARY_METHOD(name, func)                                         \
  __host__ __device__ Vec2 name(const T scalar) const {                        \
    return Vec2(func(x, scalar), func(y, scalar));                             \
  }                                                                            \
                                                                               \
  __host__ __device__ Vec2 name(const Vec2 & other) const {                    \
    return Vec2(func(x, other.x), func(y, other.y));                           \
  }

#define VEC2_UNARY_METHOD(name, func)                                          \
  __host__ __device__ Vec2 name() const { return Vec2(func(x), func(y)); }
...
struct Vec2 {
  ...
  VEC2_BINARY_ASSIGNMENT_OPERATOR(operator-=, -);
  VEC2_BINARY_ASSIGNMENT_OPERATOR(operator+=, +);
  VEC2_BINARY_ASSIGNMENT_OPERATOR(operator/=, /);
  VEC2_BINARY_ASSIGNMENT_OPERATOR(operator*=, *);
...
#define VEC3_BINARY_OPERATOR(name, op)                                         \
  template <typename T>                                                        \
  __host__ __device__ Vec3<T> name(const Vec3<T> & v, const T scalar) {        \
    return Vec3<T>(v.x op scalar, v.y op scalar, v.z op scalar);               \
  }                                                                            \
                                                                               \
  template <typename T>                                                        \
  __host__ __device__ Vec3<T> name(const T scalar, const Vec3<T> & v) {        \
    return Vec3<T>(scalar op v.x, scalar op v.y, scalar op v.z);               \
  }                                                                            \
                                                                               \
  template <typename T>                                                        \
  __host__ __device__ Vec3<T> name(const Vec3<T> & v0, const Vec3<T> & v1) {   \
    return Vec3<T>(v0.x op v1.x, v0.y op v1.y, v0.z op v1.z);                  \
  }

#define VEC3_BINARY_ASSIGNMENT_OPERATOR(name, op)                              \
  __host__ __device__ Vec3 & name(const T scalar) {                            \
    return *this = *this op scalar;                                            \
  }                                                                            \
                                                                               \
  __host__ __device__ Vec3 & name(const Vec3 & other) {                        \
    return *this = *this op other;                                             \
  }

#define VEC3_BINARY_METHOD(name, func)                                         \
  __host__ __device__ Vec3 name(const T scalar) const {                        \
    return Vec3(func(x, scalar), func(y, scalar), func(z, scalar));            \
  }                                                                            \
                                                                               \
  __host__ __device__ Vec3 name(const Vec3 & other) const {                    \
    return Vec3(func(x, other.x), func(y, other.y), func(z, other.z));         \
  }
...
struct Vec3 {
  ...
  __host__ __device__ static Vec3 zero() { return Vec3<T>(0, 0, 0); }

  __host__ __device__ static Vec3 unit_x() { return Vec3<T>(1, 0, 0); }

  __host__ __device__ static Vec3 unit_y() { return Vec3<T>(0, 1, 0); }

  __host__ __device__ static Vec3 unit_z() { return Vec3<T>(0, 0, 1); }
  ...
  __host__ __device__ Vec3(const Vec2<T> & vector, T z)
  ...
  __host__ __device__ Vec3 unit() const { return *this / norm(); }
  ...
  VEC3_BINARY_ASSIGNMENT_OPERATOR(operator-=, -);
  VEC3_BINARY_ASSIGNMENT_OPERATOR(operator+=, +);
  VEC3_BINARY_ASSIGNMENT_OPERATOR(operator/=, /);
  VEC3_BINARY_ASSIGNMENT_OPERATOR(operator*=, *);
...
struct Quat {
  ...
  __host__ __device__ static Quat
  from_angle_axis(const float angle, const Vec3f & axis) {
    return Quat(
        cosf(angle / 2),
        axis.x * sinf(angle / 2),
        axis.y * sinf(angle / 2),
  ...
  __host__ __device__ Quat(float w, float x, float y, float z)
      : w(w), x(x), y(y), z(z) {}
...
struct Plane {
  ...
  Plane(const Vec3f & pivot, const Vec3f & normal)
      : pivot(pivot), normal(normal) {
...
struct Background {
  ...
  Background(
      ...
      : plane(plane), texture(texture), texture_scale(texture_scale),
        ...
        mirrors{
            Plane(
                ...
                Quat::from_angle_axis(
                    ...
                    Vec3f::unit_y()) *
                    Quat::from_angle_axis(
                        mirror_bend_angle_radians, Vec3f::unit_x()) *
                    Vec3f::unit_z()),
            Plane(
                -Vec3f::unit_z(),
                Quat::from_angle_axis(
...
__device__ Vec2z unravel_index(const size_t index, const size_t extent) {
  return Vec2z(index / extent, index % extent);
}
...
struct Trace {
  ...
  __host__ __device__ Trace()
      : ray(Vec3f::zero(), Vec3f::unit_y()), color(0, 0, 0), complete(true) {}

  __host__ __device__ Trace(const Ray & ray)
      : ray(ray), color(1, 1, 1), complete(false) {}
};
...
__global__ void trace_init_kernel(
  ...
  const Vec3f pixel_world =
      camera.position +
      Vec3f(
          R[0][0] * pixel_camera.x + R[0][1] * pixel_camera.y +
              R[0][2] * pixel_camera.z,
          R[1][0] * pixel_camera.x + R[1][1] * pixel_camera.y +
              R[1][2] * pixel_camera.z,
          R[2][0] * pixel_camera.x + R[2][1] * pixel_camera.y +
              R[2][2] * pixel_camera.z);
  traces(pixel.x, pixel.y) = Trace(Ray(
      camera.position, Vec3f::from_to(camera.position, pixel_world).unit()));
}
...
intersection_with_plane(const Ray & ray, const Plane & plane) {
  const float denominator = plane.normal.dot(ray.direction);
  return (
      equals_approx(denominator, 0)
          ? Intersection()
          : Intersection(
                Vec3f::from_to(ray.origin, plane.pivot).dot(plane.normal) /
                denominator));
}

__device__ Intersection intersection_with_medium(
    const float scattering_coefficient, curandStatePhilox4_32_10 & rng) {
...
__device__ float perceived_luminance(const Vec3f & color_bgr) {
  ...
  return (
      0.0722f * color_bgr.x + 0.5870f * color_bgr.y + 0.2990f * color_bgr.z);
}
...
__global__ void trace_kernel(
  ...
  const Intersection intersections[4] = {
      ...
      scene.third_mirror_present
          ? intersection_with_plane(trace.ray, scene.mirrors[2])
          : Intersection(),
  };
  ...
  const Intersection intersection_medium = intersection_with_medium(
      scene.medium_scattering_coefficient, rng_state(pixel.x, pixel.y));
  ...
  if (contact_index == BACKGROUND) {
    ...
    const float cos_angle_of_incidence =
        fabsf(trace.ray.direction.dot(scene.background.plane.normal));

    trace.color *= cos_angle_of_incidence *
                   (texel_color * scene.background_radiance).min(1);
    trace.complete = true;
    ...
    ray_count(pixel.x, pixel.y) += 1;
      ...
      contact_index == MIRROR_0 || contact_index == MIRROR_1 ||
      contact_index == MIRROR_2) {
    trace.color *= scene.mirror_albedo;

    constexpr float DARK_THRESHOLD = 1 / 255.f;

    if (trace.color.x < DARK_THRESHOLD && trace.color.y < DARK_THRESHOLD &&
...
cudu::host::Array3D<unsigned char>
kaleido::raytraced_image(const Config & config) {
  const cv::Mat texture_cv =
      cv::imread(config.background_texture_path, cv::ImreadModes::IMREAD_COLOR);
  ...
  const Scene scene(
      ...
      Vec2f(
          config.background_texture_offset[0],
          config.background_texture_offset[1]),
  ...
  const Vec3f camera_position =
      (Quat::from_angle_axis(
           radians(config.camera_position_azimuth_angle_degrees),
           Vec3f::unit_y()) *
       Quat::from_angle_axis(
           radians(config.camera_position_polar_angle_degrees),
           Vec3f::unit_x()) *
       Vec3f(0, 1, 0))
          .unit() *
      config.camera_position_radius;
  ...
  const auto workload =
      cudu::Workload::for_jobs(unsigned(image_resolution.size()));

  cudu::device::Array2D<Stratifier> stratifiers(
      image_resolution, Stratifier(config.stratifier_resolution));
  ...
  CUDU_LAUNCH(
      rng_state_setup_kernel, workload, config.stratifier_rng_seed, rng);
  ...
  if (config.debug) {
    ...
    for (unsigned count : h_ray_count) {
      ray_count_mean += count;
    }
    ray_count_mean /= h_ray_count.size();

    float ray_count_std = 0;
    for (unsigned count : h_ray_count) {
      ray_count_std += abs(count - float(ray_count_mean));
    }
    ray_count_std /= h_ray_count.size();

    const float cpu_s = float(end_clock - start_clock) / CLOCKS_PER_SEC;

    std::cout << std::fixed << std::setprecision(2) << "CPU time: " << cpu_s
              << " s\n"
